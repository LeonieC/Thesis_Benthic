### Bayesian Model with Hard Coral Cover ####
...
# if want to contain only "Hard Coral" we defined
HC.cover <- subset (BC.cover, MajorCategory == "Scleractinian")
HC.cover <- aggregate (Cover ~ Region + Site + Depth + Transect + Quadrat + MajorCategory + Genus, HC.cover , sum)

HC.cover_df <- HC.cover

# Modify Coral Cover from per picture 100% to per site 100%
HC.cover <- ddply(HC.cover, ~ Region + Site + Depth + MajorCategory + Genus, function(x){c(Cover = sum(x$Cover))})
HC.cover <- HC.cover %>%
...
HC.cover_df <- aggregate (Cover ~ Region + Site + Quadrat +  Depth, HC.cover_df , sum)
...
Binomial_HC.Cover_model <- brm(Coral_points | trials(Tot_Points) ~  Depth + (1 | Site),
...
save(Binomial_HC.Cover_model, file="/Users/leonie/Desktop/R_for_Benthic/Benthic_R/Binomial_HC.Cover_model_Diversity.RData")
load("/Users/leonie/Desktop/R_for_Benthic/Benthic_R/Binomial_HC.Cover_model_Diversity.RData")
...
summary (Binomial_HC.Cover_model)
bayes_R2(Binomial_HC.Cover_model)

# The best is to use the Binomial_Cover_Model ####
plot(Binomial_HC.Cover_model)
...
pp_check(Binomial_HC.Cover_model, type = "scatter_avg") # Not structured data
bayes_R2(Binomial_HC.Cover_model) # 
r2_bayes(Binomial_HC.Cover_model)

HC.me_null <- conditional_effects(Binomial_HC.Cover_model, nsamples = 1000, probs = c(0.05, 0.95), spaghetti = F) # Default is 0.95
...
Depth <- unique (Binomial_HC.Cover_model$data$Depth)
Site <- unique (Binomial_HC.Cover_model$data$Site)
Coral_points <- unique (Binomial_HC.Cover_model$data$Coral_points)
Tot_Points <- unique (Binomial_HC.Cover_model$data$Tot_Points)
#
HC.ref_data <- crossing(Depth, Site, Tot_Points)

# Forest plots with the posteriors
HC.fitted_values <- posterior_epred(Binomial_HC.Cover_model, newdata = HC.ref_data, re_formula = 'Coral_points | trials(Tot_Points) ~  Depth + (1 | Site)')

# Number of rows equals to HC.ref_data and number of dimensions is equal to (Site*Depths=27)
# str (HC.fitted_values)
# dim (HC.fitted_values)

# Necessary to traspose
HC.fitted_values <- t(HC.fitted_values)

# Create combination of HC.ref_data
HC.ref_data_fitted <- cbind (HC.ref_data [c(1,2)],HC.fitted_values)

# Multiple columns of predictions into a single one. 
HC.ref_data_fitted <- melt (HC.ref_data_fitted, id.vars = c ("Depth", "Site"), na.rm = F, measure.vars = c(3:6002), value.name = c("Posterior_Prob"))
HC.ref_data_fitted$Depth = factor (HC.ref_data_fitted$Depth, levels = c ("30", "15","5"))


# Summary depth and island among all iterations 

HC.summary <- ddply(HC.ref_data_fitted, .(Depth,Site), summarize, Post_Mean=mean(Posterior_Prob), Post_Sd = sd(Posterior_Prob), Post_se=sd(Posterior_Prob) / sqrt(length(Posterior_Prob)), 
                 Post_Margin.error = qt(p=0.05/2, df=length (Posterior_Prob)-1,lower.tail=F) * Post_se)

HC_cover2 <- merge (HC_cover2, HC.summary)
...
HC_cover2$Coral_points <- (HC_cover2$Cover * HC_cover2$Tot_Points) / 100
# Round the points
HC_cover2$Coral_points <-round(HC_cover2$Coral_points,0)
HC_cover2$NonCoral_points <-abs (HC_cover2$Coral_points - HC_cover2$Tot_Points)


# Transform the posterior predict of binomial from points proportions (out of 25) to coral cover (%)
HC_cover2$Post_Mean_Cover <- (HC_cover2$Post_Mean * 100) / HC_cover2$Tot_Points
HC_cover2$Post_Sd_Cover <- (HC_cover2$Post_Sd * 100) / HC_cover2$Tot_Points
HC_cover2$Post_se_Cover <- (HC_cover2$Post_se * 100) / HC_cover2$Tot_Points
HC_cover2$Post_Margin.error_Cover <- (HC_cover2$Post_Margin.error * 100) / HC_cover2$Tot_Points
...
HC2_cover <- HC_cover2
HC2.summary <- ddply(HC2_cover, .(Depth), summarize, Mean=mean(Cover), Cover_se=sd(Cover) / sqrt(length(Cover)), 
...
# Combine dataframes
HC2_cover <- merge (HC2_cover,HC2.summary)

# Make a single value for each depth of Post Bayesian
HC2.summary2 <- ddply(HC2_cover, .(Depth), summarize, Post_Cover_Depth=mean(Post_Mean_Cover), Post_Cover_sd_Depth = sd (Post_Mean_Cover))
HC2_cover <- merge (HC2_cover,HC2.summary2)


# Make the plot again
HC2_cover$Depth <- as.numeric (as.character(HC2_cover$Depth))



# Plot with Bayesian Binomial model (with standard deviation of the model and with coral cover mean values with CI

HC.Fig_1A <- ggplot(HC2_cover, aes(x=Depth, y=Cover)) +
  ...
  scale_y_continuous(name ="Hard Coral cover (%)", limits=c(-5,55), breaks = c(0,10,20,30,40,50)) +
...
ggsave("HC_Fig_1A.png", plot = HC.Fig_1A, width = 2.76, height = 2, dpi = 500)
